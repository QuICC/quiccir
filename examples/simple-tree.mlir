...
func.func @entry(%thisArr: !llvm.array<1x!llvm.ptr>, %Polv: !type_umod, %Rv: !type_uval, %Thetav: !type_uval, %Phiv: !type_uval) {
  %R = builtin.unrealized_conversion_cast %Rv : !type_uval to tensor<?x?x?xf64>
  %Theta = builtin.unrealized_conversion_cast %Thetav : !type_uval to tensor<?x?x?xf64>
  %Phi = builtin.unrealized_conversion_cast %Phiv : !type_uval to tensor<?x?x?xf64>
  %Polt = call @simpleTree(%R, %Theta, %Phi) : (tensor<?x?x?xf64>, tensor<?x?x?xf64>, tensor<?x?x?xf64>) -> tensor<?x?x?xf64>
  %Pol = builtin.unrealized_conversion_cast %Polt : tensor<?x?x?xf64> to !type_tumod
