...
SmallVector<Value, 2> getIdxPtr(Operation *op,
  ...
  if (isa<TransposeOp>(op)) {
    ...
    Value ptrStruct =
        rewriter.create<LLVM::ExtractValueOp>(loc, metaArray, indexPtr);
    Type I32Type = rewriter.getI32Type();
    Type memTy = MemRefType::get({ShapedType::kDynamic}, I32Type);
    SmallVector<Value, 1> ptrOperands = {ptrStruct};
    Value ptr =
        rewriter.create<UnrealizedConversionCastOp>(loc, memTy, ptrOperands)
            ->getResult(0);
    Value idxStruct =
        rewriter.create<LLVM::ExtractValueOp>(loc, metaArray, indexIdx);
    SmallVector<Value, 1> idxOperands = {idxStruct};
    Value idx =
        rewriter.create<UnrealizedConversionCastOp>(loc, memTy, idxOperands)
            ->getResult(0);
    return {ptr, idx};
  } else {
    // Otherwise we get ptr and idx from producer view
    Type i32Type = IntegerType::get(op->getContext(), 32);
    Type metaTy = MemRefType::get({ShapedType::kDynamic}, i32Type);
...
struct OpLowering : public ConversionPattern {
  OpLowering(MLIRContext *ctx, TypeConverter &typeConverter)
      : ConversionPattern(typeConverter, Top::getOperationName(), /*benefit=*/1,
                          ctx) {}
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    // However, if the consumer is quicc.materialize we simply write in that
    // buffer
    auto genRetBuffer =
        [&](Operation *op) -> llvm::Expected<SmallVector<Value, 3>> {
      ...
      for (auto indexedResult : llvm::enumerate(op->getResults())) {
        ...
        if (!useExistingBuffer) {
          ...
          Value ldsVal = rewriter.create<LLVM::ConstantOp>(
              loc, I64Type, rewriter.getI64IntegerAttr(lds));

          Type dataTy = MemRefType::get({ShapedType::kDynamic},
                                        retViewTy.getElementType());
          Value data = rewriter.create<AllocDataOp>(
              loc, dataTy, ptrIdx[0], ptrIdx[1], ldsVal,
              retViewTy.getEncoding().cast<StringAttr>().str());
          Value buffer = rewriter.create<AssembleOp>(loc, retViewTy, ptrIdx[0],
                                                     ptrIdx[1], data);
          buffers.push_back(buffer);
    ...
    if (!cast<Top>(*op).getImplptr()) {
      return rewriter.notifyMatchFailure(op,
                                         "Implementation attribute is missing");
    }
    SmallVector<int64_t, 1> index = {
        static_cast<int64_t>(cast<Top>(*op).getImplptr().value())};
    ...
    for (auto ret : retBuffers) {
      Value newOp =
          rewriter.create<UnrealizedConversionCastOp>(loc, retTensorType, ret)
              ->getResult(0);
...
void QuiccirToCallLoweringPass::runOnOperation() {
  ...
  target.addLegalOp<quiccir::MaterializeOp, quiccir::PointersOp,
                    quiccir::IndicesOp, quiccir::AllocDataOp,
                    quiccir::AssembleOp>();
  ...
  patterns.add<OpLowering<quiccir::AddOp>>(&getContext(), viewConverter);
  patterns.add<OpLowering<quiccir::SubOp>>(&getContext(), viewConverter);
