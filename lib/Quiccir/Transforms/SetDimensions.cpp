...
namespace {
struct QuiccirSetDimensions
    : public QuiccirSetDimensionsBase<QuiccirSetDimensions> {
  using QuiccirSetDimensionsBase<
      QuiccirSetDimensions>::QuiccirSetDimensionsBase;

  QuiccirSetDimensions(llvm::ArrayRef<int64_t> phys,
...
QuiccirSetDimensions::QuiccirSetDimensions(llvm::ArrayRef<int64_t> phys,
                                           llvm::ArrayRef<int64_t> mods) {
  for (auto &&p : phys) {
    _phys.push_back(p);
  }
...
void setMissingDimensions(Value val, llvm::ArrayRef<std::int64_t> dims) {
  ...
  if (auto tensor = valTy.dyn_cast<RankedTensorType>()) {
    ...
    auto plusDymsTy =
        get<RankedTensorType>(val.getContext(), newShape, eleTy, encoding);
...
std::unique_ptr<Pass>
mlir::quiccir::createSetDimensionsPass(llvm::ArrayRef<int64_t> phys,
                                       llvm::ArrayRef<int64_t> mods) {
