...
namespace {
struct QuiccirSetViewLayout
    : public QuiccirSetViewLayoutBase<QuiccirSetViewLayout> {
  using QuiccirSetViewLayoutBase<
      QuiccirSetViewLayout>::QuiccirSetViewLayoutBase;
...
void setMissingLayout(Value val, llvm::StringRef enc) {
  ...
  if (auto tensor = valTy.dyn_cast<RankedTensorType>()) {
    if (!tensor.getEncoding()) {
      ...
      auto plusAttrTy =
          get<RankedTensorType>(val.getContext(), shape, eleTy, encoding);
