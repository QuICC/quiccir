...
QuiccirToStructConverter::QuiccirToStructConverter() {
  ...
  addConversion([&](ViewType view) -> Type { return convertView(view); });
  addConversion(
      [&](MemRefType memRef) -> Type { return convertMemRef(memRef); });
...
mlir::Type QuiccirToStructConverter::convertView(ViewType view) {
  ...
  structElementTypes.push_back(
      mlir::LLVM::LLVMArrayType::get(ctx, i32Type, view.getShape().size()));
  /// meta pos
  ...
  if (auto complex = dyn_cast<mlir::ComplexType>(dataPtr)) {
    ...
    structElementTypes.push_back(mlir::LLVM::LLVMPointerType::get(dataPtr));
  }
  structElementTypes.push_back(i32Type);
  /// struct
...
TensorToViewConverter::TensorToViewConverter() {
  addConversion([](Type type) { return type; });
  addConversion(
      [&](mlir::RankedTensorType view) -> Type { return convertTensor(view); });
}

mlir::Type TensorToViewConverter::convertTensor(mlir::RankedTensorType tensor) {
  return ViewType::get(tensor.getShape(), tensor.getElementType(),
                       tensor.getEncoding());
}