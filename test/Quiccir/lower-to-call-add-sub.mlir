...
module {
    ...
    // CHECK: func.func @entrySubBufUse(%arg0: !llvm.ptr<array<6 x ptr<struct<(ptr, ptr, i64, array<1 x i64>, array<1 x i64>)>>>>, %{{.*}}: !llvm.ptr<array<1 x ptr>>, %[[C:.*]]: !quiccir.view<16x2x3xf32, "layoutIn">, %[[A:.*]]: !quiccir.view<16x2x3xf32, "layoutIn">, %{{.*}}: !quiccir.view<16x2x3xf32, "layoutIn">) {
    ...
    // CHECK: %[[ARR:.*]] = llvm.load %{{.*}} : !llvm.ptr<array<1 x ptr>>
    // CHECK: %[[THIS:.*]] = llvm.extractvalue %[[ARR]][0] : !llvm.array<1 x ptr>
    // CHECK: %[[CNODIM:.*]] = builtin.unrealized_conversion_cast %[[C]] : !quiccir.view<16x2x3xf32, "layoutIn"> to !quiccir.view<?x?x?xf32, "layoutIn">
    // CHECK: call @_ciface_quiccir_sub_f32_layoutIn_f32_layoutIn_f32_layoutIn(%[[THIS]], %[[CNODIM]], %{{.*}}, %{{.*}}) : (!llvm.ptr, !quiccir.view<?x?x?xf32, "layoutIn">, !quiccir.view<?x?x?xf32, "layoutIn">, !quiccir.view<?x?x?xf32, "layoutIn">) -> ()
    ...
    // CHECK: %[[VIEW:.*]] = quiccir.assemble(%[[PTR]], %[[IDX]]), %[[DATA]] : (memref<?xi32>, memref<?xi32>), memref<?xf32> -> !quiccir.view<16x2x3xf32, "layoutIn">
    // CHECK: %[[VIEWNODIM:.*]] = builtin.unrealized_conversion_cast %[[VIEW]] : !quiccir.view<16x2x3xf32, "layoutIn"> to !quiccir.view<?x?x?xf32, "layoutIn">
    // CHECK: %[[ANODIM:.*]] = builtin.unrealized_conversion_cast %[[A]] : !quiccir.view<16x2x3xf32, "layoutIn"> to !quiccir.view<?x?x?xf32, "layoutIn">
    // CHECK: call @_ciface_quiccir_sub_f32_layoutIn_f32_layoutIn_f32_layoutIn(%{{.*}}, %[[VIEWNODIM]], %[[ANODIM]], %{{.*}}) : (!llvm.ptr, !quiccir.view<?x?x?xf32, "layoutIn">, !quiccir.view<?x?x?xf32, "layoutIn">, !quiccir.view<?x?x?xf32, "layoutIn">) -> ()
