...
module {
  // CHECK: func.func @entryAddFr(%[[A0:.*]]: tensor<?x?x?xcomplex<f64>>, %[[A1:.*]]: tensor<?x?x?xcomplex<f64>>) -> tensor<?x?x?xf64> {
  // CHECK: %[[ADD:.*]] = quiccir.add %[[A0]], %[[A1]] : tensor<?x?x?xcomplex<f64>>, tensor<?x?x?xcomplex<f64>> -> tensor<?x?x?xcomplex<f64>>
  ...
  %ftphys0 = quiccir.fr.prj %ftmod0 : tensor<?x?x?xcomplex<f64>> -> tensor<?x?x?xf64> attributes {kind = "P"}
  %ftmod1 = quiccir.transpose %alphys1 permutation = [1, 2, 0] : tensor<?x?x?xcomplex<f64>> -> tensor<?x?x?xcomplex<f64>>
  // CHECK: %[[FT:.*]] = quiccir.fr.prj %{{.*}} : tensor<?x?x?xcomplex<f64>> -> tensor<?x?x?xf64> attributes {kind = "D1"}
  %ftphys1 = quiccir.fr.prj %ftmod1 : tensor<?x?x?xcomplex<f64>> -> tensor<?x?x?xf64> attributes {kind = "D1"}
  // CHECK: %[[RET:.*]] = quiccir.add %{{.*}}, %[[FT]] : tensor<?x?x?xf64>, tensor<?x?x?xf64> -> tensor<?x?x?xf64>
  %add = quiccir.add %ftphys0, %ftphys1 : tensor<?x?x?xf64>, tensor<?x?x?xf64> -> tensor<?x?x?xf64>
  // CHECK: return %[[RET]] : tensor<?x?x?xf64>
  return %add : tensor<?x?x?xf64>
  }

  // CHECK: func.func @entrySubAl(%[[A0:.*]]: tensor<?x?x?xcomplex<f64>>, %[[A1:.*]]: tensor<?x?x?xcomplex<f64>>) -> tensor<?x?x?xcomplex<f64>> {
  // CHECK: %[[SUB:.*]] = quiccir.sub %[[A0]], %[[A1]] : tensor<?x?x?xcomplex<f64>>, tensor<?x?x?xcomplex<f64>> -> tensor<?x?x?xcomplex<f64>>
